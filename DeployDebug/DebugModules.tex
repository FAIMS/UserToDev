...
\placefigure[force][USB]{Use a USB cable to connect your device to computer (Mug, lunchbox, and dice are optional).}{\externalfigure[image20.jpg][width=.5\textwidth]}
...
On your computer, open up the command window (on Windows, press the Windows key, and search for \quotation{cmd}). Using the change directory ({\tt cd}) and {\tt ls} to list directory contents, navigate to where you installed the android-sdk.

\placefigure[force][navigate-directory]{Use {\tt ls} to check that the directory has the android-sdk installed.}{\externalfigure[image18.png][width=.5\textwidth]}

While in this directory, type in {\tt monitor} and hit enter. Wait a few minutes and this command will open the Android Debug Monitor, a graphical program you can use to view messages and information from any attached Android devices and emulated devices.

\placefigure[force][debug-monitor]{The Android Debug Monitor appears after some waiting.}{\externalfigure[image63.png][width=.5\textwidth]}
...
The bottom portion of the Android Debug Monitor displays different messages from your applications running on your Android device. This is sometimes known as the \quotation{LogCat} window. When several applications are running, the window will quickly fill up with messages and it may be hard to keep track of them all. Luckily, you can limit the displayed messages using the filter box. To see only messages from the FAIMS app, type in {\tt app: faims} and hit enter. Now only messages from the FAIMS app will appear in the window.

\placefigure[force][limit-log]{Limiting messages to those from the FAIMS app.}{\externalfigure[image38.png][width=.5\textwidth]}

You can limit messages in the Android Debug Monitor by using the filter box to search for \quotation{pid}; \quotation{app}; \quotation{tag}, or \quotation{text}.

\subsection[identifying-log-messages]{Identifying Log Messages}

Logs are messages developers create to complete when apps complete certain actions or when special cases arise that the programmer anticipated. To search from log messages from applications, type {\tt text: Log} into the LogCat search field.
...
To further limit your LogCat output to show just error messages, you can click the dropdown menu to the right of the search field. By default, LogCat shows all messages with the \quotation{verbose} option. You can also select to show just debug, info, warn, or assert messages from the dropdown menu as well.

\placefigure[force][error-only]{Use the drop down menu to display only error messages.}{\externalfigure[image61.png][width=.5\textwidth]}
...
\placefigure[force][save-logs]{Logs can be shared in text file format.}{\externalfigure[image3.png][width=.5\textwidth]}
...
\quiz{Test your knowledge}{
	...
	\item Exercise: Try searching the LogCat messages using a different filter, like \quotation{text:} or \quotation{tag:}. What messages appear or disappear depending on the filter? Can you find another way to filter out messages from just the \quotation{FAIMS} app?
}
