...
\placefigure[force][server-IP]{Enter \quotation{http://} and the IP address to access your faims server.}{\externalfigure[image28.png][width=.5\textwidth]}

Next, log into the FAIMS server using either the default admin account or an account that you set up (we recommend setting up individual accounts for each user). {\bf Reminder:} The default FAIMS account is {\tt faimsadmin@intersect.org.au}, password {\tt Pass.123}.
...
\item Click \quotation{Show Modules}
\item Click \quotation{Create Module}
\item Give your Module a name, and enter as much metadata as possible.
\item Upload the Data Definition Schema ({\tt data_schema.xml}).
\item Upload the User Interface Schema ({\tt ui_schema.xml})
\item Upload the User Interface Logic ({\tt ui_logic.bsh})
\item Upload the Translation (Arch16n) file ({\tt faims.properties})
\stopitemize
