...
Notice that the topmost {\tt <User>} tag contains the {\em attribute} {\tt f} with an {\em attribute value} of {\tt nodata}. Including the word {\tt nodata} in the {\tt f} attribute's value prevents the FAIMS-Tools from automatically generating unwanted code associated with the data schema. In practical terms: inputs that are provided in the \quotation{User} tab group, including the \quotation{Select User} dropdown menu, are not considered \quotation{data} that must be saved to the FAIMS database. We chose to put {\tt nodata} here because we only want the \quotation{User} tab group to be for letting people log in.

Now that we've got our {\tt <User>} tab group, it's time to make our actual {\tt <User>} tab with its relevant GUI elements. We're naming this tab \quotation{User} as well, because it's where the User logs in and that seems like a good name, but we could have named it something else if we thought we'd get it confused with the tab group. Whatever tag name we choose will be displayed in the list of tabs. When you name a tab group something, its tag name must be capitalized.

The {\tt <Select_User>} and {\tt <Login>} elements represent GUI elements, or the actual things users will interact with when they're viewing a tab. Notice the {\tt t} attribute, which is where we can define part of what this element looks like or does. You'll find a list of these in the FAIMS cookbook. For {\tt <Select_User>} we're using {\tt t="dropdown"}, which means this GUI element is a drop-down menu. 

So how do we determine what's in that drop down menu? In this case, by creating an attribute, {\tt f="users"}, which in this context FAIMS understands to mean \quotation{get the list of usernames from the server and put them here.}\footnote{{\bf Note from FAIMS programmer Christian Nassif-Haynes:} If you use {\tt t=dropdown}, then it is possible for the user to avoid logging in (in error or intentionally) by selecting the null option and clicking the login button. If you want to prevent logging with the null user option (de facto avoiding the login), then you need to manually modify the {\tt ui_logic.bsh} file after it's been generated or use {\tt t=list}, following the guidelines below. Lists, unlike dropdowns, do not allow null elements so using a {\tt t="list"} for login, as in the code below, prevents the problem of logging in with null user.
...
~~</User> 
 ...
 Setting {\tt t="button"}, as in the case of {\tt <Login>}, creates a button you can tap. The purpose of the button is described by the text that comes after it; otherwise, it's just a button, which probably isn't going to be very useful for your module. Here we've included the code {\tt l="Control"} within the tag, which causes FAIMS to link to the \quotation{Control} tab group when it is tapped. In fact, the {\tt l} attribute works not only for buttons, but many other GUI elements as well. Note carefully that the \quotation{Control} tab group linked to by the {\tt l} attribute's value is defined further down in the {\tt module.xml} file; this code only functions because the destination is valid. If we had {\tt 1="Control"} and then didn't actually have a \quotation{Control} to link to, it's safe to say this wouldn't work. Also note that references are case-sensitive, so writing {\tt control} with a lower-case \quotation{c} would fail.
...
The {\tt <Main>} element creates the first tab. Inside that tab we have a GUI element, {\tt Record_Interview.} The {\tt t="button"}, which means this element is a button; {\tt 1="Interview"}, so the button links to another tab somewhere else in the module called \quotation{Interview.} The other GUI element here is \quotation{GPS Diagnostics,} which has the element type {\tt gpsdiag.} This element type specifically means that in the final module, it will create text labels and display information about the Android device's GPS location. In the screenshot, we see the {\tt gpsdiag} element at work: it's telling us that our phone's GPS is \quotation{not initialized,} a charming way of sayingn \quotation{not turned on.} Until we do turn it on, this is the best {\tt gpsdiag} is going to do.
...
With the GPS antenna turned on, the {\tt gpsdiag} element now displays quite a bit more information, including the status of the GPS antenna, location in both Latitude/Longitude and Easting/Northing, and an accuracy measurement.

So much for the {\tt <Main>} tab. Let's look at the next tab, which you may have already noted is a little peculiar. For one thing, the tag name begins with a lowercase letter. Didn't we tell you never to do that? Furthermore, the text contained within, \quotation{Records,} isn't in tags. What's going on here?
...
For the {\tt <Title>} element, there is no UI \quotation{type} (represented by {\tt t=}) specified. So it becomes the default \quotation{type;} a text input field. (See \goto{{\em Type Guessing for GUI Elements in FAIMS-Tools}}[type-guessing-for-gui-elements-in-faims-tools] for an explanation of how this was determined.) The flag {\tt f=notnull} designates that this is a required field; the record cannot be saved if it is empty. If you can imagine your stress levels skyrocketing because of teammembers not remembering to enter in their (X), set the relevant element to {\tt f=notnull} and they will have no choice but to remember. 
...
\placecode[here][codePrivate]{XML for {\tt <Private>}.}{
\startXML
<Private t="radio" f="notnull">
	<desc>Choose either "false", meaning that the metadata for the item should be publicly available, or "true", meaning that the metadata for the item should be hidden (perhaps because you plan to check it and edit it later).</desc>
	<opts>
...
This next element (\in{Codeblock}[codeAgentRole]), {\tt <Add_Agent_Role>}, designates a button element ('t="button"') that links to the tab group {\tt Agent_Role.} This will allow users to register new Agent Roles.

\placecode[here][codeAgentRole]{XML for {\tt <Add_Agent_Role>.}}{
\startXML
<Add_Agent_Role t="button" lc="Agent_Role"/>
\stopXML
}

Note that this time, instead of using an {\tt l} to redirect elsewhere, {\tt lc} was used. The difference is that using {\tt lc} instead of {\tt l} establishes a parent-child relationship, with the entity linking becoming a parent and the entity being linked to becoming a child. This is useful for organizing your module's data in a neat, hierarchical way. 

\in{Codeblock}[codeAgentRoleDD] designates a dropdown menu, {\tt <List_of_Agent_Roles>}, which is populated with a list of {\tt Agent_Role} records. Specifically, they will be the {\tt Agent_Role} records which were saved using the button element above. The FAIMS-Tools knows these are the right records to display because the button and dropdown menu appear in the same tab group.

\placecode[here][codeAgentRoleDD]{XML for {\tt List_of_Agent_Roles}.}{
\startXML
<List_of_Agent_Roles t="dropdown" ec="Agent_Role"/>
\stopXML
}

The final two element types used in this tab are the {\tt t="audio"} and {\tt t="video"} types (\in{Codeblock}[codeAttachAV]). These two element types allow you to record audio and video files and attach them to your records.
...
As an exercise, follow these instructions and produce your own copy of the module's code in {\tt module.xml}. Then save that to the server and run the generate.sh script to produce necessary files. Go to \quotation{create module} and upload the necessary files to the server as the \quotation{Simple Sample Module.} 
